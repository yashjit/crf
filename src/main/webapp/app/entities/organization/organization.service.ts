...
export class OrganizationService {
  ...
  protected convertDateFromClient(organization: IOrganization): IOrganization {
    const copy: IOrganization = Object.assign({}, organization, {
      createDate: organization.createDate && organization.createDate.isValid() ? organization.createDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((organization: IOrganization) => {
        organization.createDate = organization.createDate ? moment(organization.createDate) : undefined;
      });
    }
    return res;
  }
